...
MaTi <- read.csv('Sara_SarahSarahEatenByWolves.csv', header=TRUE, sep=',', 
                 na.strings=c('','NA'))
...
# there was an error in previous regex where 'lands R mt kick to low L back of knee'
# would get counted as a knee strike, this version of code should fix it.

kicks_sq1 <- grep('land.*kick', sq1)
elbows_sq1 <- grep('land.*elbow', sq1)

#excludes string with zero or more characters after knee
...
#added_received is an extension of the original Added, but with the added_landed, 
#added_missed, and added_received binary/dummy columns for actions/reactions
...
Added2 <- Added[,-2] #removes SecondsIntoRound
Seconds <- mutate(Added2, SecondsIntoRound=300-(as.numeric(Added2$Time)))
seconds <- Seconds[,c(1,181,2:180)]

Sec <- as.data.frame(seconds$SecondsIntoRound)
colnames(Sec) <- 'previousAction'

#add a zero obs for this field to bind to the previousAction field
initial <- as.data.frame(0)
colnames(initial) <- 'previousAction'

lastAction <- rbind(initial,Sec)
lastAction <- lastAction[1:162,]
seconds$lastAction <- lastAction
seconds <- seconds[,c(1:2,182,3:181)]
seconds <- seconds[,-4] #remove the empty SecondsLastRoundAction field

#extension of seconds using mutate() of dplyr library, keeps value of seconds into round as
#seconds since last action if no action that second timed observation
last <- mutate(seconds, SecondsLastRoundAction=if_else(seconds$SecondsIntoRound-seconds$lastAction>0,
                                                       seconds$SecondsIntoRound-seconds$lastAction,
                                                       seconds$SecondsIntoRound))
last <- last[,c(1:3,182,4:181)] #reorders so that SecondsLastRoundAction is at front fields location


##########################################################################################
################### Check these tables for the calculations -- that they are correct #####
...
# Create break points where last action in seconds is greater than the amount of seconds of the round
# This is because there are three round 1's from three fights, where the cut off needs to be in place
# for the end of a round when counting seconds since last action of that round.
...
# into the round for each of the three table splits just created above.
...
write.csv(Table8, 'Nunez-ML-Analysis-editedRegex.csv', row.names=F)
